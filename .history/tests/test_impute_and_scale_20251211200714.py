...
def test_missing_imputer_scaler_fit(tmp_path, scaler_imputer_dataframe):
    ...
    preprocessor = DataScalerAndImputer(
        ...
        random_state=42,
        n_neighbors=2,
        cols_to_use=scaler_imputer_dataframe.columns.tolist(),
    )
    # rodar pipeline
    preprocessor.run()
    ...
    assert (
        preprocessor.X_train_imputed[preprocessor.missing_columns]
        .isnull()
        .sum()
        .sum()
        == 0
    ...
    assert (
        preprocessor.X_test_imputed[preprocessor.missing_columns]
        .isnull()
        .sum()
        .sum()
        == 0
